...
//static int linestate=0, linex=0; 
...
/* ULA */
  ...
  while(1)
  {
    ...
    radsave = radjust;
    if(intsample && nmi_pending)
    {
      ts = nmi_interrupt();
    }
    else if (intsample && int_pending)
    {
      ts += z80_interrupt();
...
void checkhsync(int tolchk)
{
	     ...
	     (  tolchk &&                                                         RasterX>=HSYNC_TOLERANCEMAX ) )
	{
		...
		RasterY++;
#ifdef DEBUG_PRINT        
        if (print_debug && RasterY == 203)
          debug_tracking = 1;
        else if (print_debug && RasterY == 206)
          debug_tracking = 0;
#endif
